# Real Conference Photos — content map

Originals from `~/Documents/Chosen Conference/photos/`. Optimised here at 800/1280/1920 widths in JPG + WebP.

| File stem | Content | Vibe | Suggested placement |
|---|---|---|---|
| `dsc00143` | Two young men celebrating in front of UPPERROOM 25 ribbon backdrop | Joyful · communal · branded | Image-mosaic; tile-grid featured (Talks/Joy) |
...
| `dsc00196` | Sister + crowd in hall — smiling religious sister seated with attendees | Communal · warm · human-led | **CTA-banner** (front + about) |
...
| `img_4837` | Group of 9 young people, peace signs, arms around | Community · "Be Radiant" group | **Stat-strip photo bg**; image-mosaic hero |

**Sizes per file:** `{stem}-800.{jpg,webp}`, `{stem}-1280.{jpg,webp}`, `{stem}-1920.{jpg,webp}`.

**Use `<picture>` with `<source srcset>` for WebP, JPG fallback.** WebP is smaller; JPG remains for older clients.

When real CHOSEN 2026 photography is shot, replace files in place — keep stems the same so block markup needn't change.
